...
package com.beibeishow.nimoreport.slidemenu;

import com.beibeishow.nimoreport.slidemenu.SlideMenu.CanvasTransformer;
import com.beibeishow.nimoreport.slidemenu.SlideMenu.OnClosedListener;
import com.beibeishow.nimoreport.slidemenu.SlideMenu.OnOpenOrCloseListener;
import com.beibeishow.nimoreport.slidemenu.SlideMenu.OnOpenedListener;
 ...
 * 
 */
public class SlideMenuAbove extends ViewGroup {

    private static final String TAG = "SlideMenuAbove";
    ...
    private SlideMenuBehind mViewBehind;
    ...
    private OnOpenOrCloseListener mOnOpenOrCloseListener;
    ...
    public SlideMenuAbove(Context context) {
        this(context, null);
    }

    public SlideMenuAbove(Context context, AttributeSet attrs) {
        super(context, attrs);
        initCustomViewAbove();
    }
    ...
    public void setOnOpenOrCloseListener(OnOpenOrCloseListener l) {
        mOnOpenOrCloseListener = l;
    }
    ...
    public void setCustomViewBehind(SlideMenuBehind cvb) {
        mViewBehind = cvb;
    }
    ...
    private void openOrClose() {
        if (isMenuOpen()) {
            ...
            if (mOnOpenOrCloseListener != null) {
                mOnOpenOrCloseListener.slideToRight(true);
                //TODO
                //CountUtil.onUmengEvent(getContext(), ConstantCount.ON_ENTER_SLIDEMENU);
            }
        } else {
            if (mOnOpenOrCloseListener != null) {
                mOnOpenOrCloseListener.slideToLeft(true);
            }
    ...
    protected int mTouchMode = SlideMenu.TOUCHMODE_MARGIN;
    ...
    private boolean thisTouchAllowed(MotionEvent ev) {
        ...
        if (isMenuOpen()) {
            ...
            switch (mTouchMode) {
                case SlideMenu.TOUCHMODE_FULLSCREEN:
                    return !isInIgnoredView(ev);
                case SlideMenu.TOUCHMODE_NONE:
                    return false;
                case SlideMenu.TOUCHMODE_MARGIN:
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        /*
         * if (!mEnabled) return false; final int action = ev.getAction() &
         * MotionEventCompat.ACTION_MASK; Log.d(TAG, "Received ACTION_DOWN"); if
         * (action == MotionEvent.ACTION_CANCEL || action ==
         * MotionEvent.ACTION_UP || (action != MotionEvent.ACTION_DOWN &&
         * mIsUnableToDrag)) { endDrag(); return false; }
         */

        switch (ev.getAction()/* action */) {
            case MotionEvent.ACTION_MOVE:
                ...
                try {
                    ...
                    Log.d(TAG, "onInterceptTouch moved to:(" + x + ", " + y + "), diff:(" + xDiff
                            + ", " + yDiff + "), mLastMotionX:" + mLastMotionX);
                    if (xDiff > mTouchSlop && xDiff > yDiff && thisSlideAllowed(dx)) {
                        Log.d(TAG, "Starting drag! from onInterceptTouch");
                        startDrag();
                        mLastMotionX = x;
                        setScrollingCacheEnabled(true);
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (action & MotionEventCompat.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                ...
                break;
            case MotionEvent.ACTION_MOVE:
                if (/* SlidePagerView.firstScreen == 0 && */!mIsBeingDragged) {
                ...
                if (mIsBeingDragged) {
                    ...
                    scrollXMove = oldScrollX + deltaX;
                    final float leftBound = getLeftBound();
                    final float rightBound = getRightBound();
                    if (scrollXMove < leftBound) {
                        scrollXMove = leftBound;
                    } else if (scrollXMove > rightBound) {
                        scrollXMove = rightBound;
                    }
                    // Don't lose the rounded component
                    mLastMotionX += scrollXMove - (int) scrollXMove;
                    scrollTo((int) scrollXMove, getScrollY());
                    pageScrolled((int) scrollXMove);
                }
                float alpha = (Math.abs(ev.getX()) / 1000) * 1.5f;
                mViewBehind.setAlpha(alpha);
                break;
    ...
    public void scrollTo(int x, int y) {
        ...
        ((SlideMenu) getParent()).manageLayers(getPercentOpen());
